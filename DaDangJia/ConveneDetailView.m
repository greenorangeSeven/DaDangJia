...
@implementation ConveneDetailView
...
- (void)getJoinData
{
    ...
    if ([UserModel Instance].isNetworkRunning) {
        ...
        [param setValue:self.topic.topicId forKey:@"topicId"];
        NSString *findRhTopicUserPageByTopicIdUrl = [Tool serializeURL:[NSString stringWithFormat:@"%@%@", api_base_url, api_findRhTopicUserPageByTopicId] params:param];
        
        [[AFOSCClient sharedClient]getPath:findRhTopicUserPageByTopicIdUrl parameters:Nil
                                   success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                       @try {
                                           NSLog(@"%@", operation.responseString);
                                           NSData *data = [operation.responseString dataUsingEncoding:NSUTF8StringEncoding];
...
- (IBAction)joinAction:(id)sender {
    ...
    [param setValue:self.topic.topicId forKey:@"topicId"];
    [param setValue:userInfo.regUserId forKey:@"regUserId"];
    NSString *joinAndCancelTopicUrl =[Tool serializeURL:[NSString stringWithFormat:@"%@%@", api_base_url, api_joinAndCancelTopic] params:param];
    [[AFOSCClient sharedClient]getPath:joinAndCancelTopicUrl parameters:Nil
                               success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                   @try {
                                       NSLog(@"%@", operation.responseString);
                                       NSData *data = [operation.responseString dataUsingEncoding:NSUTF8StringEncoding];
